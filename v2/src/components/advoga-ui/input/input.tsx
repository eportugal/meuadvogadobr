  ...
  ({ 
    ...
    return (
      <div className="grid gap-2">
        {label && (
          <Label htmlFor={inputId} className="text-sm font-medium text-input-label">
            {label}
          </Label>
        )}
        <div className="relative">
          ...
          <input
            ...
            className={cn(
              ...
              "min-h-[40px] max-w-full sm:max-w-xs",
              LeftIcon && "pl-10",
              (rightIcon || isRightIconComponent) && "pr-10",
              error && "border-input-border-error focus:border-input-border-error focus:ring-input-border-error/20",
              className
            )}
            ref={ref}
            {...props}
          ...
          {(RightIcon || isRightIconComponent) && (
            <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center">
              {isRightIconComponent ? (
                ...
                RightIcon && <RightIcon className="size-4 text-muted-foreground pointer-events-none" />
              )}
            </div>
          )}
        </div>
        {error && (
          <p className="text-xs text-input-text-error mt-1">{error}</p>
        )}
      </div>
    );
  }
